...
static uint8_t s_rx_buf[ BSP_OBD_RX_BUF_SIZE ];
static uint8_t s_tx_buf[ BSP_OBD_TX_BUF_SIZE ];
...
void bsp_uart_obd_init( void )
{
    ...
    gpio_set_speed( BSP_OBD_TX_PORT, BSP_OBD_TX_PIN, GPIO_SPEED_LOW );  /* 10.4kbps */
    ...
    uart_driver_init( &s_obd,
                      BSP_UART_CH_OBD,
                      s_rx_buf, BSP_OBD_RX_BUF_SIZE,
                      s_tx_buf, BSP_OBD_TX_BUF_SIZE );

    nvic_enable_irq( BSP_OBD_IRQn, BSP_OBD_IRQ_PRIORITY );
}
